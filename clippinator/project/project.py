...
class Project:
    ...
    ci_commands: dict[str, str] = field(
        default_factory=dict
    )  # keys: 'run', 'lint', 'lint_file', 'test'
    ...
    def get_folder_summary(
        ...
        if len(res) > length_3:
            print(
                f"Warning: long project summary at {path}, truncating to {length_3} chars"
            )
    ...
    def lint(self, path: str = ""):
        ...
        if self.ci_commands.get("lint"):
            ...
            return trim_extra(
                process.stdout.strip() + process.stderr.strip(), 3000, end_length=1500
            )
    ...
    def menu(self, prompt=None):
        from clippinator.tools.utils import get_input_from_editor, select

        prompt_options = ["Edit action summary"] * (prompt is not None)
        res = select(
            ["Continue", "Architecture", "Objective", "Memories", "CI"]
            + prompt_options,
            "Project Menu",
        )
        if res == 1:
            ...
            ci_commands = get_input_from_editor(
                "\n".join([f"{k}: `{v}`" for k, v in self.ci_commands.items()])
            ).splitlines()
            self.ci_commands = {
    ...
    def prompt_fields(self) -> dict:
        ...
        return {
            ...
            "memories": "  - " + "\n  - ".join(self.memories),
            "architecture_example": templates.get(self.template, {}).get(
